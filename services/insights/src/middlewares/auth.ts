...
interface User { user_id: number; name: string; email: string; role: "jobseeker" | "recruiter"; subscription: string | null; }
...
export const isAuth = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const users = await sql`SELECT user_id, name, email, role, subscription FROM users WHERE user_id = ${decoded.id}`;
