...
export const endorseSkill = TryCatch(async (req: AuthenticatedRequest, res) => {
  const endorser = req.user!;
  const { userId } = req.params;
  const { skill_name } = req.body;

  if (endorser.user_id === Number(userId)) throw new ErrorHandler(400, "Cannot endorse your own skill");
  if (!skill_name) throw new ErrorHandler(400, "skill_name is required");

  const [existing] = await sql`
    SELECT endorsement_id FROM skill_endorsements
    WHERE endorsed_user_id = ${userId} AND endorser_id = ${endorser.user_id} AND skill_name = ${skill_name}
  `;
  if (existing) throw new ErrorHandler(409, "Already endorsed this skill");

  await sql`
    INSERT INTO skill_endorsements (endorsed_user_id, endorser_id, endorser_name, skill_name)
    VALUES (${userId}, ${endorser.user_id}, ${endorser.name}, ${skill_name})
  `;
  res.status(201).json({ message: "Skill endorsed" });
});

export const getEndorsements = TryCatch(async (req, res) => {
  const { userId } = req.params;
  const endorsements = await sql`
    SELECT skill_name, COUNT(*)::int AS count,
      json_agg(json_build_object('endorser_id', endorser_id, 'endorser_name', endorser_name)) AS endorsers
    FROM skill_endorsements WHERE endorsed_user_id = ${userId}
