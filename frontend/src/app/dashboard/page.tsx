...
const DashboardPage = () => {
  ...
  useEffect(() => {
    if (!authLoading && (!isAuth || user?.role !== "recruiter")) {
      router.push("/");
    }
    ...
    <div className="min-h-screen bg-secondary/30">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
          ...
          <Link href="/account">
            <Button variant="outline" className="gap-2"><Users size={16} /> My Profile</Button>
          </Link>
        </div>
        ...
        {/* Application breakdown + hire rate */}
        ...
        <Card className="border-2 p-6 mb-8">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold flex items-center gap-2">
              <BarChart3 size={18} className="text-blue-600" /> Hire Rate
            </h2>
          ...
          <p className="text-xs opacity-50 mt-2">
            {stats.total_hired} hired out of {stats.total_applications} total applications
          </p>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div key={a.application_id} className="p-4 flex items-center justify-between gap-3 hover:bg-accent transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs opacity-50">
                      {a.company_name} · {new Date(a.applied_at).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
                    </p>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <Link href={`/account/${a.applicant_id}`} target="_blank">
                      <ExternalLink size={13} className="opacity-40 hover:opacity-100" />
                    </Link>
                  </div>
                </div>
              )) : (
          ...
          <Card className="border-2 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-5 flex items-center justify-between">
              <h2 className="text-white font-semibold flex items-center gap-2">
                <TrendingUp size={18} /> Top Jobs by Applications
              </h2>
              ...
              {topJobs.length > 0 ? topJobs.map((j, i) => (
                <div key={j.job_id} className="p-4 flex items-center gap-3 hover:bg-accent transition-colors">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${j.is_active ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-500"}`}>
