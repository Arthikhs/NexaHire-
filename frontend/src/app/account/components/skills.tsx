...
const Skills: React.FC<AccontProps> = ({ user, isYourAccount }) => {
  ...
  const removeSkillHandler = (skillToRemove: string) => {
    if (confirm(`Are you sure you want to remove ${skillToRemove} ?`)) removeSkill(skillToRemove);
  };
      ...
      <Card className="shadow-lg border-2 overflow-hidden">
        <div className="bg-blue-500 p-6 border-b">
          <div className="flex items-center gap-3 mb-4">
            ...
            {isYourAccount && (
              <CardDescription className="text-sm mt-1 text-white">Showcase your expertise and abilities</CardDescription>
            )}
          </div>
        </div>

        {isYourAccount && (
          <div className="flex gap-3 flex-col sm:flex-row p-4">
            <div className="relative flex-1">
              ...
              <Input type="text" placeholder="e.g. React, Node.js, Python..."
                className="h-11 pl-10 bg-background" value={skill}
                onChange={(e) => setSkill(e.target.value)} onKeyPress={handleKeyPress} />
            </div>
            ...
            <>
              {isYourAccount && (
                <div className="flex flex-wrap gap-3">
                  {user.skills.map((e, i) => (
                    <div key={i} className="group relative inline-flex items-center gap-2 border-2 rounded-full hover:shadow-sm duration-200 transition-all pl-4 pr-3 py-2">
              ...
              <SkillEndorsements profileUserId={user.user_id} skills={user.skills} />
            </>
          ) : (
